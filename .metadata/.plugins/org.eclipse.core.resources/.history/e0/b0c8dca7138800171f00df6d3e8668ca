	...
	private void readConfigurations() {
		try {
			...
			while((line=br.readLine())!=null && !line.trim().isEmpty()) {
				...
				if(line.startsWith("obstacleDistance")) this.obstacleDistance = Integer.parseInt(st.nextToken().trim());
			}
			br.close();
		} catch(Exception e) {
			e.printStackTrace();
			System.out.println("Error reading configurations.txt file!");
			System.exit(1);
		}
